...
export const useMilkStore = create((set, get) => ({
  ...
  upsertEntry: async (data) => {
    try {
      ...
      const errorMsg = err.response?.data?.error || 'Failed to save entry';
      set({ error: errorMsg });
      throw err;
    } finally {
      set({ loading: false });
  ...
  getEntry: async (farmId, animalId, date) => {
    try {
      ...
      const errorMsg = err.response?.data?.error || 'Failed to fetch entry';
      set({ error: errorMsg });
      throw err;
    } finally {
      set({ loading: false });
  ...
  listByDate: async (farmId, date) => {
    try {
      ...
      const errorMsg = err.response?.data?.error || 'Failed to fetch entries';
      set({ error: errorMsg });
      throw err;
    } finally {
      set({ loading: false });
